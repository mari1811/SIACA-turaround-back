...
class MaquinariaHistorial(APIView):
    ...
    def get(self, request, fecha=None, horaI=None, horaF=None, *args, **kwargs):
        ...
        if token:
            ...
            maquinarias = maquinaria_historial.objects.filter(fecha = fecha).filter(hora_inicio__range = (hourI, hourF)).all() | maquinaria_historial.objects.filter(fecha = fecha).filter(hora_fin__range = (hourI, hourF)).all()
            if maquinarias:
